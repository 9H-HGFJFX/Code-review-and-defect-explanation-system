...
$mysqlArgs = @('-u','root','-h','127.0.0.1','-P','3307','--connect-timeout=5','-B')

function Query($sql) {
    $args = @('-u','root','-h','127.0.0.1','-P','3307','--connect-timeout=5','-B','--default-character-set=utf8mb4','-e',$sql)
    $result = & 'C:\Program Files\MySQL\MySQL Server 8.0\bin\mysql.exe' @args 2>&1
    return $result
}

$classesRaw = Query 'SELECT id,name,teacher_id,IFNULL(description,"") FROM code_review.class ORDER BY id'
$rulesRaw = Query 'SELECT id,rule_id,name,description,category,severity,IFNULL(languages,""),IFNULL(pattern,""),enabled,priority FROM code_review.review_rule ORDER BY priority'
$tasksRaw = Query 'SELECT id,title,IFNULL(description,""),class_id,submitter_id,status,IFNULL(deadline,""),IFNULL(created_at,"") FROM code_review.review_task ORDER BY id'

# Parse classes
$classes = @()
foreach ($line in $classesRaw) {
    $parts = $line -split "`t"
    Write-Host "DEBUG: [$line] parts=$($parts.Count)"
    if ($parts.Count -ge 4) {
...
.badge{display:inline-block;padding:6px 14px;background:rgba(255,255,255,.2);border-radius:20px;font-size:13px;font-weight:500;backdrop-filter:blur(10px)}
...
.flow-step{flex:1;min-width:160px;background:#fff;padding:20px 15px;border-radius:12px;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.05);position:relative;transition:transform .2s}
...
.cat-best_practice{background:#d3f9d8;color:#2f9e44}
...
$rulesHtml = ""
foreach ($r in $rules) {
    $catKey = ($catMap[$r.category]).ToLower().Replace('_','')
    $catKey = switch ($r.category) { 1 {'style'} 2 {'security'} 3 {'performance'} 4 {'bestpractice'} 5 {'correctness'} default {'style'} }
    $sevKey = switch ($r.severity) { 1 {'critical'} 2 {'high'} 3 {'medium'} 4 {'low'} default {'medium'} }
    $catName = $catMap[$r.category]
    $sevName = $sevMap[$r.severity]
    $rulesHtml += @"
<div class="card">
<h3>$($r.name)</h3>
<div class="meta">
...
$tasksHtml = ""
foreach ($t in $tasks) {
    $statusName = $statMap[$t.status]
    $statusClass = $statClass[$t.status]
    $tasksHtml += @"
<tr>
<td class="id">#$($t.id)</td>
